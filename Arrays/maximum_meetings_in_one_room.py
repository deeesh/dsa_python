...
class Solution:
    def maxMeetings(self, N : int, S : List[int], F : List[int]) -> List[int]:
        # code here
        create_stp = [[S[i], F[i], i+1] for i in range(N)]
        select = []
        create_stp.sort(key=lambda x:x[1])
        # print(create_stp)
        select.append(create_stp[0][2])
        end_key = create_stp[0][1]
        for i in range(1,N):
            if end_key < create_stp[i][0]:
                select.append(create_stp[i][2])
                end_key = create_stp[i][1]
        select.sort()
        return select
